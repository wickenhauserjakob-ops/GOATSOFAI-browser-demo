...
async function loadLabels() {
  const response = await fetch("../labels.txt?v=7", { cache: "no-store" });
  if (!response.ok) {
...
async function loadModel() {
  ...
  try {
    ...
    const modelResponse = await fetch("../model.tflite?v=7", { method: "HEAD", cache: "no-store" });
    ...
    try {
      ...
      model = await runtime.loadTFLiteModel("../model.tflite?v=7");
      ...
      const modelBody = await fetch("../model.tflite?v=7", { cache: "no-store" });
      if (!modelBody.ok) {
