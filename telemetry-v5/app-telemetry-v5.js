...
function updateScanStats(totalMs, timings, detections) {
  telemetry.runs += 1;
...
async function loadLabels() {
  const response = await fetch("../labels.txt?v=5", { cache: "no-store" });
  if (!response.ok) {
...
async function loadModel() {
  ...
  try {
    ...
    const modelResponse = await fetch("../model.tflite?v=5", { method: "HEAD", cache: "no-store" });
    ...
    try {
      ...
      model = await runtime.loadTFLiteModel("../model.tflite?v=5");
      ...
      const modelBody = await fetch("../model.tflite?v=5", { cache: "no-store" });
      if (!modelBody.ok) {
...
async function runInference() {
  ...
  try {
    ...
    updateScanStats(elapsed, timings, detections);
