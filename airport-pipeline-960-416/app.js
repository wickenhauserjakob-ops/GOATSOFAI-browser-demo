...
let scanLog = [];
let debugLog = [];
const LOG_STORAGE_KEY = "goatsofai-airport-pipeline-log-v1";
const MAX_LOG_ENTRIES = 500;

const telemetry = {
  build: "airport-pipeline-v2-rectcrop-2026-07-10",
...
async function loadModel() {
  ...
  try {
    setStatus("Loading pipeline models...");
    ...
    variantModel = await loadTflite(runtime, "variant.tflite");
...
function analyzeVariant(output) {
  ...
  for (const name of ["A320", "737-200", "A319", "A321", "737-300"]) {
    ...
    watchScores[name] = index >= 0 ? decoded.classScores[index] : null;
  }
  return {
    detections,
    topClasses,
    ...
    bestCandidate: topClasses[0] || null,
  };
}
...
function buildReport() {
  return JSON.stringify({
    model: "GOATSOFAI airport two-stage browser pipeline",
    pipeline: "tracker 960 TFLite -> crop original camera frame -> variant 416 TFLite",
